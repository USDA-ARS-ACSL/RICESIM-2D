	...
	Character*10 Sowing,Emerge,Transplant,Ending,Date4,Date1  ! date1 is dummy for beginDay until we modify the itnerface
...
c     for writing frequency to output
      Daily=0
      Hourly=0
      AutoIrrigateF=0 !no autoirrigate option in intials yet for rice, set to 0
      cContentRootM=0.40
      cContentRootY=0.40
      nContentRootM=0.012
      nContentRootY=0.19
         ...
         date1='00/00/0000'
          ...
          iapos = Quote_Count(inDates)
            if (iapos.eq.4) then
               ...
               read(inDates,*,err=8)  Sowing, Emerge, Transplant, 
     &              Ending, TimeStep 
             else 
                goto 8
              endif 
        ...
        read(41,*,err=8)
         im=im+1         
        read(41,*,err=8) RICETYPE
         im=im+1
        read(41,*,err=8)
         im=im+1         
        read(41,*,err=8) RUNMODE
         im=im+1
        read(41,*,err=8)
         im=im+1 
        read(41,*,err=8) PRODENV
         im=im+1
        read(41,*,err=8)
         im=im+1         
        read(41,*,err=8) WATBAL
         im=im+1
        read(41,*,err=8)
         im=im+1            
        read(41,*,err=8) NITROENV
         im=im+1
        read(41,*,err=8)
         im=im+1            
        read(41,*,err=8)
        ...
        read(41,*,err=8)
         im=im+1  
        read(41,*,err=8) NPLH, NHH, NPLSB, NPLDS
         im=im+1           
        read(41,*,err=8)
         im=im+1            
        read(41,*,err=8)
        ...
        read(41,*,err=8)
         im=im+1  
        read(41,*,err=8) LAPE, DVIS, WLVGI, WSTI, WRTI, WSOI, ZRTI
         im=im+1  
        read(41,*,err=8)
         im=im+1            
        read(41,*,err=8)
        ...
        read(41,*,err=8)
         im=im+1  
        read(41,*,err=8) SWITIR
         im=im+1           
c SWITIR refers to rainfed or different irrigation options, this would be handled in AUTOIrrig or Irrigation subroutines eventually         
       Close(41)
...
c         if (date1.ne.'00/00/0000') beginDay=JulDay(date1)
         ...
         transplantDay=JulDay(Transplant)
